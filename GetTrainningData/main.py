...
def generate_questions(text):
    ...
    completion = client.chat.completions.create(
        ...
        {"role": "user", "content": "Generate 5 questions based on the following text: " + text}
        ]
    )
...
if __name__ == "__main__":
    ...
    with open('InfoExtractFromPDF\\extractedInfo\\Processed Christ-Centered Exposition Commentary.jsonl', 'r') as file:
        # Iterate over the lines in the file
        for index, line in enumerate(file):
            # Parse the line as a JSON object
            obj = json.loads(line)

            print("=============== Processing paragraph #:", obj.get("Paragraph #"), "===============")

            # Skip the object if it is empty or too short
            text = obj.get("Text")
            if len(text) < 170:
                continue
